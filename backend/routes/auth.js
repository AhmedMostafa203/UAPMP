...
const sendVerificationEmail = async (email, userId, userName) => {
  const verificationUrl = `${process.env.BACKEND_URL || "https://university-academic-productivity-management-plat-production.up.railway.app"}/api/auth/verify-email/${userId}`;
...
router.post("/verify-email", async (req, res) => {
  try {
    ...
    if (!uid) {
      return res
        .status(400)
        .json({ message: "Verification uid is required", code: "MISSING_UID" });
    }
    ...
    const buildRedirect = (user) => {
      ...
      const dashboardUrl =
        user.role === "student"
          ...
          : process.env.INSTRUCTOR_DASHBOARD_URL ||
